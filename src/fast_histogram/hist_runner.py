...
from fast_histogram.hist_wrapper import create_hist_wrappers, BinarySearchHist_name, UnimMeshHist_name, NumPyHist_name

implementations = None

def run_histograms(a, bins, device, implementations, dtype="float32"):

    results = dict()
    for key, impl in implementations.items():
        if impl.is_device_supported(device):
            print(f"{key} {device}...", end=' ', flush=True)
            a_dev = impl.from_numpy(a, dtype, device)
            bins_dev = impl.from_numpy(bins, dtype, device)
            impl.prepare(bins_dev)
            impl(a_dev)
...
def get_device_implementations(device, verbose=False):
    ...
    if verbose or implementations is None:
        implementations = create_hist_wrappers(verbose)
